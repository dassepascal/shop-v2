...
class extends Component {
	...
	public function saveMenu(): void
	{
		...
		$this->success(__('Menu created with success.'), redirectTo: '/admin/blog/menus/index');
	}
        ...
        @foreach ($menus as $menu)
            <x-list-item :item="$menu" no-separator no-hover>
                ...
                <x-slot:actions>
                    ...
                    <x-popover>
                        <x-slot:trigger>
                            <x-button icon="c-arrow-path-rounded-square" link="{{ route('admin.blog.menus.edit', $menu->id) }}"
                    ...
                    @foreach ($menu->submenus as $submenu)
                        <x-list-item :item="$menu" no-separator no-hover>
                            ...
                            <x-slot:actions>
								...
								<x-popover>
									<x-slot:trigger>
										<x-button icon="c-arrow-path-rounded-square" link="{{ route('admin.blog.submenus.edit', $submenu->id) }}"
											class="text-blue-500 btn-ghost btn-sm" spinner />
									</x-slot:trigger>
									<x-slot:content class="pop-small">
										@lang('Edit')
